...
async function submitForm(url, fieldValues) {
  ...
  try {
    ...
    const hadForm       = (await safeQuerySelector(page, "form")) !== null;
    const SUCCESS_TEXTS = [
      "送信完了", "ありがとうございました", "お問い合わせを受け付けました",
      "送信しました", "完了", "thank you", "received", "success",
      "受け付けました", "受付完了", "送付しました", "確認メール", "折り返し", "担当者", "confirmation", "thank",
      // Contact Form 7の標準完了メッセージ("ありがとうございます。メッセージは送信されました。")対応。
      // 既存の「送信しました」(能動)は「送信されました」(受動)を含まないため表記ゆれとして追加。
      // 「ありがとうございます」単体は問い合わせページの案内文(送信前から表示)と誤検知しうるため含めない。
      "送信されました", "message has been sent", "your message was sent",
    ];
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (hadForm && (await safeQuerySelector(page, "form")) === null) {
        ...
        const formReappeared = (await safeQuerySelector(page, "form")) !== null;
        if (formReappeared) {
  ...
  for (const form of forms) {
    ...
    for (const a of anchors) {
      ...
      if (isJsTrigger && SUBMIT_TEXTS.some(t => info.text.includes(t.toLowerCase()))) {
        try { await a.click(); return true; } catch { continue; }
      }
    }
  }
