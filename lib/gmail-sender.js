...
// refresh_token/emailを取得し、未指定なら環境変数のデフォルトアカウントを使う
// (後方互換性のため)。senderIdが指定されたのに見つからない/無効化されている場合は
// エラーにする(黙って別アカウントから送るのを防ぐため)。
async function resolveSender(senderId) {
  if (senderId === undefined || senderId === null) {
    return {
      refreshToken: process.env.GMAIL_REFRESH_TOKEN || null,
      email: process.env.SENDER_EMAIL || "me",
    };
  }
  const [account] = await sql`
    SELECT email, refresh_token FROM sender_accounts WHERE id = ${senderId} AND is_active = TRUE
  `;
  if (!account) {
    throw new Error("指定された送信者アカウントが見つからないか、無効化されています");
  }
  return { refreshToken: account.refresh_token, email: account.email };
}

async function getAccessToken(senderId) {
  const clientId     = process.env.GMAIL_CLIENT_ID;
  const clientSecret = process.env.GMAIL_CLIENT_SECRET;
  const { refreshToken } = await resolveSender(senderId);
...
async function sendEmail({ to, subject, body, attachment, senderId }) {
  const accessToken = await getAccessToken(senderId);
  if (!accessToken) return { configured: false };

  const { email } = await resolveSender(senderId);
  const from = email || "me";
...
// senderIdは、送信に使ったアカウントと同じアカウントにラベルを付与するために指定する
// (異なるアカウントのメールボックスを操作することはできないため)
async function ensureLabel(labelName, senderId) {
  const accessToken = await getAccessToken(senderId);
...
async function addLabelToMessage(messageId, labelId, senderId) {
  const accessToken = await getAccessToken(senderId);
