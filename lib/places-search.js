...
// 業種を問わず常に除外するノイズワード(人材派遣・求人系はどの業種を検索していても不要)
const NOISE_KEYWORDS_ALWAYS = ["派遣", "求人", "転職"];
...
function isNoisyPlaceName(name, industry) {
  if (!name) return false;
  if (NOISE_KEYWORDS_ALWAYS.some(k => name.includes(k))) return true;

  const isItSearch = industry === "IT・情報通信";
  if (!isItSearch) return false;

  if (NOISE_KEYWORDS_IT_STRICT.some(k => name.includes(k))) return true;
  if (NEAR_EXACT_NOISE_KEYWORDS_IT_STRICT.some(k => isNearExactMatch(name, k))) return true;
...
async function searchPlacesAPI(region, keyword, pageSize = 20, industry = "") {
