...
async function analyzeForm(url) {
  ...
  try {
    ...
    let forms = await extractFormsWithRetry(page);
    if (forms.length === 0 && contactUrl === null) {
      ...
      if (fallbackUrl && fallbackUrl !== formPageUrl) {
        try {
          ...
          forms = await extractFormsWithRetry(page);
        } catch {
          // 失敗時は無視
        }
      }
    ...
    if (forms.length === 0) {
      const lastResortUrl = await findContactLink(page, targetUrl, true);
    ...
    let iframeUrls = [];
    if (forms.length === 0) {
      const iframeResult = await findFormsInFrames(page).catch(() => ({ iframeForms: [], inaccessibleIframes: [] }));
    ...
    const rejection = await detectRejection(page);
    const automatable = forms.length > 0 && !captchaDetected && !rejection.detected;
    const extractedEmail = await extractEmailFromPage(page).catch(() => null);
...
async function extractForms(page) {
  return page.evaluate(extractFormsInBrowserContext);
}

// フォームが即座に見つからない場合、JavaScriptによる動的なDOM追加を考慮して
// 2.5秒待ってから再度探索する
async function extractFormsWithRetry(page) {
...
async function findFormsInFrames(page) {
  const iframeForms = [];
  ...
  for (const frame of frames) {
    ...
    try {
      const forms = await frame.evaluate(extractFormsInBrowserContext);
      if (forms.length > 0) {
        forms.forEach((f) => iframeForms.push({ ...f, iframeSrc: frameUrl }));
      }
    } catch {
      // クロスオリジン制約・フレーム未確定等でアクセスできない場合、
...
async function extractEmailFromPage(page) {
  const rawEmails = await page.evaluate(() => {
    const found = new Set();
...
async function detectRejection(page) {
