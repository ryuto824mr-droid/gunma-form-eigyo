...
// page.goto()の同種のラッパー。特にトップページへの最初のアクセスは、サイト側の
// JSリダイレクト等とレースして「Navigating frame was detached」で失敗することが
// 実運用で確認されている(Room's等のフォーム送信時と同じ原因)。一過性エラーの場合のみ
// 少し待って1回だけリトライし、それでも失敗した場合はそのまま例外を投げる
// (トップページ自体が読み込めない場合、以降の解析処理を続ける意味が無いため)
...
async function analyzeForm(url) {
  ...
  try {
    ...
    const topResponse = await safeGoto(page, targetUrl, { waitUntil: "networkidle2", timeout: 20000 });
    ...
    // 会社概要情報の抽出は、メインのフォーム解析タブが全ての遷移・操作を終えてから
    // 別タブで実行する。以前はフォーム解析と並行して別タブで実行していたが、
    // 同一ブラウザ内で2つのタブが同時にナビゲーションすると、片方の遷移がもう一方の
    // フレームを不安定にさせ、メインタブ側で「Attempted to use detached Frame」
    // 「Protocol error: Session/Target closed」が発生することが一括リサーチの
    // 再試行検証(会社間の並列度を1にしても再現)で確認されたため、逐次実行に変更した
    const companyInfo = await extractCompanyInfoSafely(browser, targetUrl);
